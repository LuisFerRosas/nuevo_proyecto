...
if __name__ == '__main__':
    dataset =obtenerDatos(pathPartituras='datos2/partituras',pathVocabulario='datos2/vocabulario2.json',
                          pathArchivoNPY='archivosnumpy/train_audio_nombre.npy',pathcomple='archivosnumpy/train_sinrecorte.npy')
    dataloader = DataLoader(dataset, batch_size=1, collate_fn=  collate_fn_nuevo,shuffle=True)
    # pbar = tqdm(dataloader)
    # variabel =next(iter(dataset))
    # print(variabel['partitura_tokenizada'].shape)
    for data in dataloader:
        partitura_tokenizada,wave_mfcc,len_partitura,wave2 = data
        ...
        print(len_partitura)
        print("len paritura //////////////// dimencion: "+str(wave2.shape))
        print(wave2)
        break
